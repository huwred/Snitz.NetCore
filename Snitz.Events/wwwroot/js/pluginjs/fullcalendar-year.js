...
newMomentProto.format = function() {
	...
	if (this._ambigTime) {
        return oldMomentFormat(this, isJalaali ? 'YYYY-M-D' : 'YYYY-MM-DD', isJalaali);
	}
	if (this._ambigZone) {
        return oldMomentFormat(this, isJalaali ? 'YYYY-M-D[T]HH:mm:ss' : 'YYYY-MM-DD[T]HH:mm:ss', isJalaali);
	}
...
var Grid = fc.Grid = RowRenderer.extend({
  ...
  //          '</th>';
        return '' +
            '<th class="fc-day-header ' + view.widgetHeaderClass + ' fc-' + dayIDs[date.day()] + '"' +
            (this.rowCnt == 1 ? ' data-date="' + date.format(view.isJalaali ? 'YYYY-M-D' : 'YYYY-MM-DD', view.isJalaali) + '"' : '') +
            '>' +
	...
	bgCellHtml: function(cell) {
		...
		return '<td class="' + classes.join(' ') + '"' +
            ' data-date="' + date.format(view.isJalaali ? 'YYYY-M-D' : 'YYYY-MM-DD',view.isJalaali) + '"' + // if date has a time, won't format it
			'></td>';
	},
	...
	getDayClasses: function(date) {
		...
		if (
			...
			(view.isJalaali ? date.jMonth() != view.intervalStart.jMonth() : date.month() != view.intervalStart.month())
		) {
		...
		dragListener = new CellDragListener(this.coordMap, {
			...
			cellOver: function(cell, isOrig, origCell) {
				...
				if (resizeLocation) {
					...
					else if (resizeLocation.start.isSame(event.start,'day') && resizeLocation.end.isSame(eventEnd,'day')) {
						resizeLocation = null;
					}
				}
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (seg.isStart) {
            ...
            timeHtml = timeText ? '<span class="fc-time"' + ' data-start="' + event.start.format(view.isJalaali) + '"' + '>' + htmlEscape(timeText) + '</span>' : '';
		}
		...
		return '<a class="' + classes.join(' ') + '"' +
				(event.url ?
					' href="' + htmlEscape(SnitzVars.baseUrl + event.url) + '"' :
					''
					) +
				(skinCss ?
					' style="' + skinCss + '"' :
...
/* A component that renders a month of events
----------------------------------------------------------------------------------------------------------------------*/
...
var MonthGrid = DayGrid.extend({
    ...
    dayRowHtml: function (row, isRigid) {
        var view = this.view;
        var classes = ['fc-row', 'fc-month', view.widgetContentClass];
        ...
        return '' +
            ...
            '</table>' +
            '</div>' +
            '<div class="fc-content-skeleton">' + //'<div class="fc-content-skeleton">' +
            '<table>' +
            '<thead>' +
    ...
    monthCellHtml: function (cell) {
        ...
        return '<td class="' + classes.join(' ') + '"' +
            ' data-date="' + date.format(view.isJalaali ? 'YYYY-M-D' : 'YYYY-MM-DD', view.isJalaali) + '"' + // if date has a time, won't format it
            '></td>';
        //<table><tr><td class="' + classes.join(' ') + '"></td></tr></table>
    },
    ...
    monthNameCellHtml: function (cell) {
        ...
        return '' +
            '<th class="' + classes.join(' ') + '" data-month="' + (this.view.isJalaali ? date.jMonth() : date.month()) + '" data-date="' + date.format(this.view.isJalaali ? 'YYYY-M-D' : 'YYYY-MM-DD', this.view.isJalaali) + '">' +
    ...
    updateCellDates: function () {
        var date = this.start.clone();
        var isjalaali = this.view.isJalaali;
        var dates = [];
        var offset = -1;
        var offsets = [];
...
var BasicView = fcViews.basic = View.extend({
	...
	numberCellHtml: function(cell) {
		...
		return '' +
            '<td class="' + classes.join(' ') + '" data-date="' + date.format(this.isJalaali ? 'YYYY-M-D' : 'YYYY-MM-DD', this.isJalaali) + '">' +
...
var YearView = fcViews.year = BasicView.extend({ // make a subclass of View
    ...
    initialize: function () {
        ...
        this.dayGrid.cellDuration = moment.duration(1, 'month');
	...
	render: function() {
        ...
        this.el.addClass('fc-year-view').html(this.renderHtml());
                        ...
                        if (!didAddDayHeader) {
                            tbody.append('\
                                <li class="my-fc-header" date="">\
                                    ...
                                    <span class="my-fc-header-date">' + (this.isJalaali ? currentDayStart.format('YYYY-M-D', this.isJalaali) : currentDayStart.format(this.opt('dateFormat'))) + '</span>\
                                </li>');
                    ...
                    if (currentDayStart.isAfter(eventEnd) || (currentDayStart.isSame(eventEnd,'day') && !eventStart.isSame(eventEnd,'day')) || periodEnd.isBefore(eventStart)) {
                        ...
                        var segEl = $('\
                            <li class="my-fc-row clearfix" data-club="' + e.club + '" data-loc="' + e.location + '">\
                                ...
                                <span class="my-fc-time text-center">' + eventStart.format("dd") + ', ' + eventStart.format(this.opt('dateFormat')) + '<br />' + e.club + '</span>\
                            </li>');
                        tbody.append(segEl);
