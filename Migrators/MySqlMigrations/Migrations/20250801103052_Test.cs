...
UPDATE FORUM_FORUM SET F_TOPICS = (SELECT T_COUNT FROM FORUM_FORUM F INNER JOIN F_T_COUNT T ON T.FORUM_ID=F.FORUM_ID);
...
UPDATE FORUM_FORUM SET F_A_TOPICS = (SELECT T_COUNT FROM FORUM_FORUM F INNER JOIN F_T_COUNT T ON T.FORUM_ID=F.FORUM_ID);
...
UPDATE FORUM_TOPICS SET T_REPLIES = (SELECT R_COUNT  FROM FORUM_TOPICS T INNER JOIN T_R_COUNT TR ON T.TOPIC_ID = TR.TOPIC_ID
   WHERE T.T_STATUS<=1);
...
UPDATE FORUM_A_TOPICS SET T_REPLIES = (SELECT R_COUNT FROM FORUM_A_TOPICS T INNER JOIN T_R_COUNT TR ON T.TOPIC_ID = TR.TOPIC_ID);
...
UPDATE FORUM_TOPICS SET T_LAST_POST = (SELECT LAST_POST FROM FORUM_TOPICS INNER JOIN T_POST_DATA PT  ON FORUM_TOPICS.TOPIC_ID = PT.TOPIC_ID);
...
UPDATE FORUM_A_TOPICS SET T_LAST_POST = (SELECT LAST_POST FROM FORUM_A_TOPICS T INNER JOIN T_POST_DATA TP  ON T.TOPIC_ID = TP.TOPIC_ID);
...
CREATE TEMPORARY TABLE T_L_A_REPLY_ID (
   TOPIC_ID int,
   REPLY_ID int
);
...
UPDATE FORUM_TOPICS SET T_LAST_POST_REPLY_ID = (SELECT REPLY_ID FROM FORUM_TOPICS T LEFT JOIN T_L_REPLY_ID TL ON TL.TOPIC_ID = T.TOPIC_ID);
...
UPDATE FORUM_TOPICS SET T_LAST_POST_AUTHOR = (SELECT AUTHOR FROM FORUM_TOPICS T INNER JOIN T_L_REPLY_AUTHOR TL ON TL.TOPIC_ID = T.TOPIC_ID);
...
UPDATE FORUM_A_TOPICS SET T_LAST_POST_AUTHOR = (SELECT AUTHOR FROM FORUM_A_TOPICS T INNER JOIN T_L_REPLY_AUTHOR TL ON TL.TOPIC_ID = T.TOPIC_ID);
...
UPDATE FORUM_FORUM SET F_COUNT = (SELECT F_COUNT + R_COUNT FROM FORUM_FORUM F INNER JOIN F_R_COUNT FR ON F.FORUM_ID = FR.FORUM_ID);
...
UPDATE FORUM_FORUM SET F_A_COUNT = (SELECT F_A_COUNT + R_COUNT FROM FORUM_FORUM F INNER JOIN F_R_COUNT FR ON F.FORUM_ID = FR.FORUM_ID);
...
UPDATE FORUM_FORUM SET F_LAST_POST = (Select LAST_POST FROM FORUM_FORUM F LEFT JOIN F_POST_DATA FP  ON F.FORUM_ID = FP.FORUM_ID);
...
UPDATE FORUM_FORUM SET F_LAST_POST_TOPIC_ID = (Select TOPIC_ID FROM FORUM_FORUM F LEFT JOIN F_TOPIC_ID FT ON F.FORUM_ID = FT.FORUM_ID);
   
/* Now Update for Author ID */
UPDATE FORUM_FORUM SET F_LAST_POST_AUTHOR=NULL;

UPDATE FORUM_FORUM F
INNER JOIN F_TOPIC_ID FT ON F.FORUM_ID=FT.FORUM_ID LEFT JOIN FORUM_TOPICS T On FT.TOPIC_ID=T.TOPIC_ID
SET F_LAST_POST_AUTHOR=T.T_LAST_POST_AUTHOR,F_LAST_POST_REPLY_ID=T.T_LAST_POST_REPLY_ID;
...
UPDATE FORUM_TOTALS FT 
INNER JOIN T_TOPICS TT ON FT.COUNT_ID=TT.COUNT_ID
SET T_COUNT = TT.TOPICS, T_A_COUNT = TT.A_TOPICS, P_COUNT=TT.POSTS, P_A_COUNT=TT.A_POSTS;
...
UPDATE FORUM_TOTALS SET U_COUNT = (SELECT MEMBERS FROM FORUM_TOTALS FT INNER JOIN T_MEMBERS TM ON FT.COUNT_ID=TM.COUNT_ID);
